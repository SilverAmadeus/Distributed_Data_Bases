--
-- ER/Studio 8.0 SQL Code Generation
-- Company :      UNAM
-- Project :      icar-global.DM1
-- Author :       Jorge
--
-- Date Created : Monday, June 05, 2017 11:24:42
-- Target DBMS : Oracle 11g
--

-- 
-- TABLE: LAPTOP_F3 
--

CREATE TABLE LAPTOP_F3(
    LAPTOP_ID                 NUMBER(10, 0)     NOT NULL,
    NUM_SERIE                 VARCHAR2(18)      NOT NULL,
    CANTIDAD_RAM              NUMBER(6, 0)      NOT NULL,
    CARACTERISTICAS_EXTRAS    VARCHAR2(2000)    NOT NULL,
    TIPO_TARJETA_VIDEO_ID     NUMBER(5, 0)      NOT NULL,
    TIPO_PROCESADOR_ID        NUMBER(5, 0)      NOT NULL,
    TIPO_ALMACENAMIENTO_ID    NUMBER(5, 0)      NOT NULL,
    TIPO_MONITOR_ID           NUMBER(5, 0)      NOT NULL,
    LAPTOP_REEMPLAZO_ID       NUMBER(10, 0),
    CONSTRAINT LAPTOP_PK PRIMARY KEY (LAPTOP_ID)
)
;



-- 
-- TABLE: LAPTOP_INVENTARIO_F3 
--

CREATE TABLE LAPTOP_INVENTARIO_F3(
    LAPTOP_ID      NUMBER(10, 0)    NOT NULL,
    RFC_CLIENTE    VARCHAR2(13),
    NUM_TARJETA    VARCHAR2(16),
    CONSTRAINT LAPTOP_INVENTARIO_PK PRIMARY KEY (LAPTOP_ID)
)
;



-- 
-- TABLE: SERVICIO_LAPTOP_F3 
--

CREATE TABLE SERVICIO_LAPTOP_F3(
    SERVICIO_LAPTOP_ID    NUMBER(10, 0)     NOT NULL,
    IMPORTE               NUMBER(8, 2)      NOT NULL,
    DIAGNOSTICO           VARCHAR2(2000)    NOT NULL,
    SUCURSAL_ID           NUMBER(10, 0)     NOT NULL,
    LAPTOP_ID             NUMBER(10, 0)     NOT NULL,
    CONSTRAINT SERVICIO_LAPTOP_PK PRIMARY KEY (SERVICIO_LAPTOP_ID)
)
;



-- 
-- TABLE: STATUS_LAPTOP_F3 
--

CREATE TABLE STATUS_LAPTOP_F3(
    STATUS_LAPTOP_ID    NUMBER(5, 0)     NOT NULL,
    CLAVE               VARCHAR2(40)     NOT NULL,
    DESCRIPCION         VARCHAR2(400)    NOT NULL,
    CONSTRAINT STATUS_LAPTOP_PK PRIMARY KEY (STATUS_LAPTOP_ID)
)
;



-- 
-- TABLE: SUCURSAL_F3 
--

CREATE TABLE SUCURSAL_F3(
    SUCURSAL_ID    NUMBER(10, 0)    NOT NULL,
    CLAVE          VARCHAR2(10)     NOT NULL,
    ES_TALLER      NUMBER(1, 0)     NOT NULL,
    ES_VENTA       NUMBER(1, 0)     NOT NULL,
    NOMBRE         VARCHAR2(40)     NOT NULL,
    LATITUD        BINARY_FLOAT     NOT NULL,
    LONGITUD       BINARY_FLOAT     NOT NULL,
    URL            VARCHAR2(200)    NOT NULL,
    CONSTRAINT SUCURSAL_PK PRIMARY KEY (SUCURSAL_ID)
)
;



-- 
-- TABLE: SUCURSAL_TALLER_F3 
--

CREATE TABLE SUCURSAL_TALLER_F3(
    SUCURSAL_ID          NUMBER(10, 0)    NOT NULL,
    DIA_DESCANSO         NUMBER(1, 0)     NOT NULL,
    TELEFONO_ATENCION    VARCHAR2(20)     NOT NULL,
    CONSTRAINT SUCURSAL_TALLER_PK PRIMARY KEY (SUCURSAL_ID)
)
;



-- 
-- TABLE: SUCURSAL_VENTA_F3 
--

CREATE TABLE SUCURSAL_VENTA_F3(
    SUCURSAL_ID      NUMBER(10, 0)    NOT NULL,
    HORA_APERTURA    DATE             NOT NULL,
    HORA_CIERRE      DATE             NOT NULL,
    CONSTRAINT SUCURSAL_VENTA_PK PRIMARY KEY (SUCURSAL_ID)
)
;



-- 
-- TABLE: TIPO_ALMAC_F3 
--

CREATE TABLE TIPO_ALMAC_F3(
    TIPO_ALMACENAMIENTO_ID    NUMBER(5, 0)     NOT NULL,
    CLAVE                     VARCHAR2(40)     NOT NULL,
    DESCRIPCION               VARCHAR2(400)    NOT NULL,
    CONSTRAINT TIPO_ALMACENAMIENTO_PK PRIMARY KEY (TIPO_ALMACENAMIENTO_ID)
)
;



-- 
-- TABLE: TIPO_MONITOR_F3 
--

CREATE TABLE TIPO_MONITOR_F3(
    TIPO_MONITOR_ID    NUMBER(5, 0)     NOT NULL,
    CLAVE              VARCHAR2(40)     NOT NULL,
    DESCRIPCION        VARCHAR2(400)    NOT NULL,
    CONSTRAINT TIPO_MONITOR_PK PRIMARY KEY (TIPO_MONITOR_ID)
)
;



-- 
-- TABLE: TIPO_PROCESADOR_F3 
--

CREATE TABLE TIPO_PROCESADOR_F3(
    TIPO_PROCESADOR_ID    NUMBER(5, 0)     NOT NULL,
    CLAVE                 VARCHAR2(40)     NOT NULL,
    DESCRIPCION           VARCHAR2(400)    NOT NULL,
    CONSTRAINT TIPO_PROCESADOR_PK PRIMARY KEY (TIPO_PROCESADOR_ID)
)
;



-- 
-- TABLE: TIPO_TARJETA_VIDEO_F3 
--

CREATE TABLE TIPO_TARJETA_VIDEO_F3(
    TIPO_TARJETA_VIDEO_ID    NUMBER(5, 0)     NOT NULL,
    CLAVE                    VARCHAR2(40)     NOT NULL,
    DESCRIPCION              VARCHAR2(400)    NOT NULL,
    CONSTRAINT TIPO_TARJETA_VIDEO_PK PRIMARY KEY (TIPO_TARJETA_VIDEO_ID)
)
;



-- 
-- TABLE: LAPTOP_F3 
--

ALTER TABLE LAPTOP_F3 ADD CONSTRAINT LAPTOP_ALMACENAMIENTO_FK 
    FOREIGN KEY (TIPO_ALMACENAMIENTO_ID)
    REFERENCES TIPO_ALMAC_F3(TIPO_ALMACENAMIENTO_ID)
;

ALTER TABLE LAPTOP_F3 ADD CONSTRAINT LAPTOP_MONITOR_FK 
    FOREIGN KEY (TIPO_MONITOR_ID)
    REFERENCES TIPO_MONITOR_F3(TIPO_MONITOR_ID)
;

ALTER TABLE LAPTOP_F3 ADD CONSTRAINT LAPTOP_PROCESADOR_FK 
    FOREIGN KEY (TIPO_PROCESADOR_ID)
    REFERENCES TIPO_PROCESADOR_F3(TIPO_PROCESADOR_ID)
;

ALTER TABLE LAPTOP_F3 ADD CONSTRAINT LAPTOP_VIDEO_FK 
    FOREIGN KEY (TIPO_TARJETA_VIDEO_ID)
    REFERENCES TIPO_TARJETA_VIDEO_F3(TIPO_TARJETA_VIDEO_ID)
;


-- 
-- TABLE: LAPTOP_INVENTARIO_F3 
--

ALTER TABLE LAPTOP_INVENTARIO_F3 ADD CONSTRAINT LAP_INVENTARIO_LAP_ID_FK 
    FOREIGN KEY (LAPTOP_ID)
    REFERENCES LAPTOP_F3(LAPTOP_ID)
;


-- 
-- TABLE: SERVICIO_LAPTOP_F3 
--

ALTER TABLE SERVICIO_LAPTOP_F3 ADD CONSTRAINT SERVICIO_LAP_ID_FK 
    FOREIGN KEY (LAPTOP_ID)
    REFERENCES LAPTOP_F3(LAPTOP_ID)
;


-- 
-- TABLE: SUCURSAL_TALLER_F3 
--

ALTER TABLE SUCURSAL_TALLER_F3 ADD CONSTRAINT SVENTA_SUCURSAL_FK 
    FOREIGN KEY (SUCURSAL_ID)
    REFERENCES SUCURSAL_F3(SUCURSAL_ID)
;


-- 
-- TABLE: SUCURSAL_VENTA_F3 
--

ALTER TABLE SUCURSAL_VENTA_F3 ADD CONSTRAINT STALLER_SUCURSAL_FK 
    FOREIGN KEY (SUCURSAL_ID)
    REFERENCES SUCURSAL_F3(SUCURSAL_ID)
;


