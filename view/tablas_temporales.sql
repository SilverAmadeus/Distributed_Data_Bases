...
create global temporary table t_insert_laptop_f5(
  laptop_id number(10,0) constraint t_rimt_insert_laptop_f5_pk  primary key,
  foto blob not null
) on commit preserve rows;

--tabla temporal para manejar blobs - transparencia para SELECT
create global temporary table t_select_laptop_f5(
  laptop_id number(10,0) constraint t_rimt_select_laptop_f5_pk  primary key,
  foto blob not null
) on commit preserve rows;
